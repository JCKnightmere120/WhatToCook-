...
export class ProfilePage {
  ...
  ionViewWillEnter(): void { this.api.profile().subscribe({ next: profile => this.fill(profile), error: () => this.message = 'Could not load your profile.' }); }
  save(): void {
    const profile: Partial<Profile> = {};
    this.fields.forEach(field => profile[field.key] = this.toArray(this.values[field.key]));
    this.saving = true; this.message = '';
    this.api.updateProfile({ ...profile, visible_to_family: this.visibility }).subscribe({ next: saved => { this.fill(saved); this.saving = false; this.message = 'Your profile has been saved.'; }, error: () => { this.saving = false; this.message = 'Could not save your profile.'; } });
  }
  private fill(profile: Partial<Profile>): void { this.fields.forEach(field => { this.values[field.key] = (profile[field.key] || []).join(', '); this.visibility[field.key] = profile.visible_to_family?.[field.key] ?? false; }); }
