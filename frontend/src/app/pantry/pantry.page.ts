...
export class PantryPage {
  ...
  suggestUnit() { if (this.form.unit) return; const name = this.form.name.toLowerCase(); if (/\b(egg|eggs)\b/.test(name)) this.form.unit = 'pieces'; else if (/\b(can|tuna|sardines|coconut milk)\b/.test(name)) this.form.unit = 'cans'; }
  ...
  lookupBarcode(barcode: string) { this.processingInput = true; this.api.barcodeInput(barcode).subscribe({ next: result => this.applyCandidates(result.candidates, result.message), error: () => { this.form.name = barcode; this.message = 'Barcode captured. Product lookup failed; enter its name and confirm the details.'; this.processingInput = false; } }); }
  startVoice() { const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition; if (!SpeechRecognition) { this.message = 'Voice input is not supported in this browser. Use Chrome on Android or enter the item manually.'; return; } const recognition = this.recognition = new SpeechRecognition(); this.voiceTranscript = ''; recognition.lang = 'en-PH'; recognition.interimResults = false; recognition.continuous = true; recognition.maxAlternatives = 1; this.listening = true; this.message = 'Recording… say one or more items, then tap Stop voice.'; recognition.onresult = (event: any) => { const transcript = Array.from(event.results).map((result: any) => result[0].transcript).join(' ').trim(); if (transcript) this.voiceTranscript = transcript; }; recognition.onerror = (event: any) => { this.listening = false; this.recognition = undefined; this.message = event.error === 'not-allowed' ? 'Microphone permission was denied. Allow it in your browser site settings and try again.' : 'Voice recording could not start. Please try again.'; }; recognition.onend = () => { this.listening = false; if (this.recognition === recognition) this.recognition = undefined; }; recognition.start(); }
  stopVoice() { if (!this.recognition) return; const transcript = this.voiceTranscript; this.recognition.stop(); this.listening = false; if (!transcript) { this.message = 'No speech was captured. Check microphone permission, then try again and speak before stopping.'; return; } this.processingInput = true; this.message = 'Processing speech…'; this.api.voiceInput(transcript).subscribe({ next: result => this.applyCandidates(result.candidates, result.message), error: () => { this.message = 'Speech was captured but could not reach the backend. Make sure Laravel is running, then try again.'; this.processingInput = false; } }); }
  uploadReceipt(event: Event) { const receipt = (event.target as HTMLInputElement).files?.[0]; if (!receipt) return; this.processingInput = true; this.api.receiptInput(receipt, this.receiptText).subscribe({ next: result => this.applyCandidates(result.candidates, result.message), error: error => { this.message = error?.error?.message || 'Could not upload the receipt.'; this.processingInput = false; } }); }
