...
export class OnboardingPage {
  ...
  submitting = false;
  error = '';
  constructor(private api: ApiService, private router: Router, private auth: AuthService, private householdContext: HouseholdContextService) {}
  createFamily(): void {
    if (!this.householdName.trim()) { this.error = 'Give your household a name first.'; return; }
    this.submitting = true; this.error = '';
    this.api.createFamily(this.householdName.trim()).subscribe({ next: family => { this.householdContext.select(this.auth.user?.id, family); this.completeSetup(); }, error: () => { this.error = 'Could not create your family account. Please try again.'; this.submitting = false; } });
  }
  joinFamily(): void {
    const code = this.joinCode.trim().toUpperCase();
    if (!/^[A-Z0-9]{8}$/.test(code)) { this.error = 'Enter the 8-character household invite code.'; return; }
    this.submitting = true; this.error = '';
    this.api.joinFamily(code).subscribe({ next: ({ family }) => { this.householdContext.select(this.auth.user?.id, family); this.completeSetup(); }, error: error => { this.error = error?.error?.message || 'Could not join this household. Check the invite code.'; this.submitting = false; } });
  }
