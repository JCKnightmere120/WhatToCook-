...
{
    public function index(Request $request) { return response()->json(Family::whereHas('members', fn ($q) => $q->where('user_id', $request->user()->id)->where('status', 'accepted'))->with(['members' => fn ($q) => $q->where('status', 'accepted')->with('user')])->get()); }
    ...
    public function invitations(Request $request) { return response()->json(['invitations' => FamilyMember::where('user_id', $request->user()->id)->where('status', 'pending')->with('family.owner')->get()]); }
    public function acceptInvitation(Request $request, FamilyMember $familyMember) { abort_unless($familyMember->user_id === $request->user()->id && $familyMember->status === 'pending', 403); $familyMember->update(['status' => 'accepted']); return response()->json(['family' => $familyMember->family->load('members.user')]); }
    ...
    private function member(Request $request, Family $family): void { abort_unless(FamilyMember::where(['family_id' => $family->id, 'user_id' => $request->user()->id, 'status' => 'accepted'])->exists(), 403); }
